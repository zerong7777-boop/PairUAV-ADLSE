...
if [[ $# -ne 1 ]]; then
  echo "Usage: $0 <alignment_run_dir>" >&2
  exit 2
fi
...
export MODEL_EXPR="Reloc3rRelpose(img_size=512, output_mode='pairuav_local_alignment_heading_range', alignment_dropout=0.0)"
...
bash scripts/eval_pairuav_official_devsplit.sh \
  ...
  /media/jgzn/SSD_lexar/RZ/UAVM/experiments/phase26_b1_geometry_local_alignment/eval/local_alignment \
  0
