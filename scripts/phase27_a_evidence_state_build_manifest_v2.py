#!/usr/bin/env python3
"""Build Phase27 A evidence-state manifest v2."""
...
IDENTITY_MANIFESTS = {
    "train_identity_manifest": "experiments/paper_pillars/15_train_test_distribution_gap/manifests/train_labeled_manifest.csv",
    "dev_identity_manifest": "experiments/paper_pillars/15_train_test_distribution_gap/manifests/dev_labeled_manifest.csv",
}
...
OUTPUT_COLUMNS = (
    ...
    + BASE_FLAG_COLUMNS
    + RISK_TAGS
    + ["ordinary_with_risk_tag", "construction_source", "schema_version", "leakage_audit_passed"]
)
...
def _anchor_stats(topk_anchors: Any) -> tuple[float | None, float | None]:
    ...
    scale_balance = 1.0 / (1.0 + anchor_spread + (max(disps) - min(disps) if len(disps) > 1 else 0.0))
    if confs:
...
def _record_to_row(record: dict[str, Any], split: str) -> dict[str, Any]:
    ...
    confidence_sum = match_count * mean_confidence if match_count is not None and mean_confidence is not None else None
    valid_ratio = None
...
def _read_features(path: Path, split: str, limit: int = 0) -> list[dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            if not stripped:
                continue
            rows.append(_record_to_row(json.loads(stripped), split))
...
def _count_lines(path: Path) -> int | None:
    ...
    if path.suffix.lower() == ".csv" and count > 0:
        return count - 1
    return count
...
def _compute_thresholds(rows: list[dict[str, Any]]) -> dict[str, float]:
    ...
    return {
        "low_match_count": _quantile(values("match_count"), 0.15, 10.0),
        "high_match_count": _quantile(values("match_count"), 0.85, 50.0),
        "low_valid_ratio": _quantile(values("valid_ratio"), 0.10, 0.20),
        "low_occupied_cells": _quantile(values("occupied_cells"), 0.15, 4.0),
        "high_occupied_cells": _quantile(values("occupied_cells"), 0.85, 10.0),
        "low_confidence_sum": _quantile(values("confidence_sum"), 0.15, 2.0),
        "high_confidence_sum": _quantile(values("confidence_sum"), 0.85, 25.0),
        "low_mean_confidence": _quantile(values("mean_confidence"), 0.10, 0.20),
        "high_mean_confidence": _quantile(values("mean_confidence"), 0.90, 0.65),
        "low_anchor_spread": _quantile(values("anchor_spread"), 0.10, 0.03),
        "high_anchor_spread": _quantile(values("anchor_spread"), 0.90, 0.30),
        "low_scale_balance": _quantile(values("scale_balance"), 0.10, 0.30),
        "high_scale_balance": _quantile(values("scale_balance"), 0.90, 0.80),
        "low_spatial_entropy": _quantile(values("spatial_entropy"), 0.10, 0.25),
...
def _write_manifest(path: Path, rows: list[dict[str, Any]], thresholds: dict[str, float]) -> dict[str, int]:
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writeheader()
        for row in rows:
            assignment = assign_evidence_state_v2(row, thresholds)
            out = dict(row)
            out.update(assignment)
            base_counts[assignment["base_regime"]] += 1
            for tag in RISK_TAGS:
                tag_counts[tag] += int(assignment[tag])
            ordinary_with_risk += int(assignment["ordinary_with_risk_tag"])
            writer.writerow({column: _format(out.get(column, "")) for column in OUTPUT_COLUMNS})
    return {
        **{f"base_{key}": value for key, value in base_counts.items()},
        **{f"risk_{key}": value for key, value in tag_counts.items()},
        "ordinary_with_risk_tag": ordinary_with_risk,
    }


def _coverage(project_root: Path, split: str, row_count: int, limit: int) -> dict[str, Any]:
    feature_counts = {
    ...
    selected_total = feature_counts.get(split)
    identity_reference = identity_counts.get("dev_identity_manifest" if split == "eval" else "train_identity_manifest")
    coverage_fraction = None
    if identity_reference:
        coverage_fraction = (selected_total or 0) / identity_reference
    return {
        "split": split,
        "limit": limit,
        "manifest_rows": row_count,
        "feature_packet_rows": feature_counts,
        "identity_manifest_rows": identity_counts,
        "selected_feature_rows_total": selected_total,
        "selected_feature_to_identity_fraction": coverage_fraction,
        "missing_observable_feature_reason": (
            "only pairs with frozen B-SCR feature packets can receive v2 observable regimes"
            if coverage_fraction is not None and coverage_fraction < 0.05
            else ""
        ),
    }
...
def main() -> int:
    ...
    write_manifest_schema_v2(args.schema_out, thresholds=thresholds, audit=audit)
    counts = _write_manifest(args.out, rows, thresholds)
    coverage = _coverage(args.project_root, args.split, len(rows), args.limit)
    if args.coverage_out:
        args.coverage_out.parent.mkdir(parents=True, exist_ok=True)
        payload = {
            "schema_version": SCHEMA_VERSION,
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "claim_level": "smoke" if args.limit else "bounded_manifest_build",
            "row_count": len(rows),
            "counts": counts,
        ...
        args.coverage_out.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
