...
LOG_PATH="$PHASE_ROOT/g2_bounded4096_pair_b2.log"
PID_PATH="$PHASE_ROOT/g2_bounded4096_pair_b2.pid"

mkdir -p "$PHASE_ROOT"
cd "$REPO_ROOT"

nohup bash scripts/phase56_run_g2_bounded4096_pair_b2.sh > "$LOG_PATH" 2>&1 &
pid="$!"
echo "$pid" > "$PID_PATH"
echo "$pid"
