...
OUT=$ROOT/experiments/phase27_a_v3_2c_bounded_outcome_consistency_reacquired_state
...
LIMIT=${LIMIT:-512}
export OUT CKPT IMAGE_ROOT LIMIT SOURCE FULL EVIDENCE TRAINING

case "$OUT" in
  "$ROOT"/experiments/phase27_a_v3_2c_bounded_outcome_consistency_reacquired_state) ;;
...
"$PY" -m scripts.phase27_a_v3_2c_state_diversity_reacquisition_diagnostic \
  --fixed-manifest "$SOURCE" \
  --bounded-manifest "$OUT/manifests/fixed_manifest_tiny.csv" \
  --evidence-manifest "$EVIDENCE" \
  --training-readiness-manifest "$TRAINING" \
  --output-dir "$OUT/reacquisition_diagnostic"

cp "$OUT/reacquisition_diagnostic/tables/bounded_manifest_with_reacquired_states.csv" "$OUT/manifests/fixed_manifest_reacquired_state.csv"
...
payload = {
    ...
    "evidence_manifest": os.environ["EVIDENCE"],
    "training_readiness_manifest": os.environ["TRAINING"],
    "limit": int(os.environ["LIMIT"]),
    "scope": "bounded fixed-manifest model forward outcome-consistency audit using exact-joined reacquired_state; no training; no full eval",
}
...
{"variant_id":"$variant","mode":"bounded_model_forward_reacquired_state","no_training":true,"limit":$LIMIT}
JSON
  "$PY" -m scripts.phase27_a_v3_2c_fixed_manifest_eval_runner \
    --fixed-manifest "$OUT/manifests/fixed_manifest_reacquired_state.csv" \
...
if metrics.get("verdict") != "fixed-manifest-runner-smoke-pass":
    (out / "reports" / "bounded_go_no_go_verdict.md").write_text(
        "# A-v3.2c Reacquired-State Bounded Outcome-Consistency Go/No-Go Verdict\n\n"
...
"$PY" -m scripts.phase27_a_v3_2c_outcome_consistency_audit \
  --manifest "$OUT/manifests/fixed_manifest_reacquired_state.csv" \
...
lines = [
    "# Phase27 A-v3.2c Reacquired-State Bounded Outcome-Consistency Experiment Record",
    "",
    ...
    f"- evidence_manifest: `{os.environ['EVIDENCE']}`",
    f"- output_dir: `{out}`",
    "",
    "## Metrics",
    "",
    ...
    f"- state_distribution: `{json.dumps(metrics.get('state_distribution', {}), sort_keys=True)}`",
...
(out / "phase27_a_v3_2c_reacquired_state_bounded_outcome_consistency_experiment_record.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
PY

for f in \
  input_manifest.json \
  manifests/fixed_manifest_tiny.csv \
  manifests/fixed_manifest_reacquired_state.csv \
  ...
  reacquisition_diagnostic/metrics/state_diversity_reacquisition_metrics.json \
  phase27_a_v3_2c_reacquired_state_bounded_outcome_consistency_experiment_record.md; do
  test -f "$OUT/$f"
done

cat "$OUT/reports/bounded_go_no_go_verdict.md"
echo A_V3_2C_REACQUIRED_STATE_BOUNDED_OUTCOME_CONSISTENCY_COMPLETE
