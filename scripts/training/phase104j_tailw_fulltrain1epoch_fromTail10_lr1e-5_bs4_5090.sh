...
UAVM_ROOT="${UAVM_ROOT:-/root/autodl-tmp/uavm_2026}"
PHASE="${UAVM_ROOT}/runs/phase88_axiswise_interaction_position_v1"
PHASE104="${UAVM_ROOT}/runs/phase104_axis_async_query_bridge_v1"
WORKER="${UAVM_ROOT}/external/reloc3r_pairuav"
if [[ -z "${PYTHON_BIN:-}" ]]; then
  if [[ -x /root/miniconda3/envs/uavm5090/bin/python ]]; then
    PYTHON_BIN=/root/miniconda3/envs/uavm5090/bin/python
...
RUN_ID="${RUN_ID:-phase104j_HR_tailw_fulltrain1epoch_fromTail10_lr1e-5_bs4_5090_20260629a}"
RUN_ROOT="${PHASE}/train_runs/${RUN_ID}"
EVAL_ROOT="${PHASE}/metrics/${RUN_ID}_val811"
RECORD_ROOT="${PHASE}/records"

INIT_CKPT="${INIT_CKPT:-${PHASE}/train_runs/phase104i_HR_tailw_fulltrain10k_fromHR50_lr1e-5_bs4_lab0628a/checkpoint-final.pth}"
...
MILESTONE_STEPS="${MILESTONE_STEPS:-50000,100000,150000,200000,250000,300000,350000,400000,450000}"
STEP_CHECKPOINT_FREQ="${STEP_CHECKPOINT_FREQ:-50000}"
MILESTONE_MODEL_ONLY="${MILESTONE_MODEL_ONLY:-0}"
...
for required in \
  ...
  if [[ ! -e "${required}" ]]; then
    echo "[phase104j-tail-1epoch] missing required path: ${required}" >&2
    exit 2
  fi
done

if [[ -e "${RUN_ROOT}" || -e "${EVAL_ROOT}" ]]; then
  echo "[phase104j-tail-1epoch] refusing to overwrite existing run or eval dir:" >&2
  echo "  run: ${RUN_ROOT}" >&2
  echo "  eval: ${EVAL_ROOT}" >&2
  exit 3
fi

mkdir -p "${RUN_ROOT}" "${EVAL_ROOT}" "${RECORD_ROOT}" "${PHASE104}/records"

{
  ...
  echo "step_checkpoint_model_only=${STEP_CHECKPOINT_MODEL_ONLY}"
  echo "print_freq=${PRINT_FREQ}"
  echo "skip_final_eval=${SKIP_FINAL_EVAL}"
  echo "interpretation=TailWeighted Phase104 PAAER/HR fulltrain 1epoch anytime long run from Phase104i Tail10 checkpoint"
} | tee "${RUN_ROOT}/phase104j_run.env"
...
if [[ "${SKIP_FINAL_EVAL}" == "1" ]]; then
  echo "date_finished=$(date -Is)" | tee -a "${RUN_ROOT}/phase104j_run.env"
  exit 0
fi
...
if [[ ! -f "${METRICS_JSON}" ]]; then
  echo "[phase104j-tail-1epoch] metrics json not found: ${METRICS_JSON}" >&2
  exit 4
fi
