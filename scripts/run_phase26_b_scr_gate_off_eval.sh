...
if [[ $# -ne 1 ]]; then
  echo "Usage: $0 <gate_off_run_dir>" >&2
  exit 2
fi
...
export MODEL_EXPR="Reloc3rRelpose(img_size=512, output_mode='pairuav_selective_correspondence_heading_range', bscr_global_dim=12, bscr_grid_size=4, bscr_topk=16, bscr_hidden_dim=128, bscr_dropout=0.0, bscr_max_residual=0.25, bscr_force_gate_off=True)"
...
export DIAGNOSTICS_OUTPUT=/media/jgzn/SSD_lexar/RZ/UAVM/experiments/phase26_b_selective_correspondence_reasoning/eval/b_scr_gate_off/diagnostics.csv
export BATCH_SIZE=4
export NUM_WORKERS=4

bash scripts/eval_pairuav_official_devsplit.sh \
  "$RUN_DIR/checkpoint-final.pth" \
  /media/jgzn/SSD_lexar/RZ/UAVM/experiments/phase26_b_selective_correspondence_reasoning/eval/b_scr_gate_off \
  0
