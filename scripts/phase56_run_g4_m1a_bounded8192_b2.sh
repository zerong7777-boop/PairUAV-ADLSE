...
RUN_NAME="${RUN_NAME:-g4_m1a_bounded8192_lab_b2_$(date +%Y%m%d_%H%M%S)}"
...
echo "phase56 G4 M1-A bounded8192 b2 started at $(date -Is)"
...
bash scripts/train_pairuav_official_metric_longer_5090.sh configs/pairuav_phase56_m1a_bounded8192_lab_b2.env

echo "phase56 G4 M1-A train completed at $(date -Is)"
nvidia-smi --query-gpu=name,memory.used,memory.total,utilization.gpu --format=csv,noheader || true
...
echo "phase56 G4 M1-A eval completed at $(date -Is)"
echo "RUN_DIR=$RUN_DIR"
echo "EVAL_DIR=$EVAL_DIR"
cat "$EVAL_DIR/val_metrics_endpoint_range_span.json"
echo "phase56 G4 M1-A bounded8192 b2 finished at $(date -Is)"
