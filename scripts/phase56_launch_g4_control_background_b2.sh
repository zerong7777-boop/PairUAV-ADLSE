...
LOG_PATH="$PHASE_ROOT/g4_control_bounded8192_b2.log"
PID_PATH="$PHASE_ROOT/g4_control_bounded8192_b2.pid"

mkdir -p "$PHASE_ROOT"
cd "$REPO_ROOT"

nohup bash scripts/phase56_run_g4_control_bounded8192_b2.sh > "$LOG_PATH" 2>&1 &
pid="$!"
echo "$pid" > "$PID_PATH"
echo "$pid"
